...
from bosshunter.db import (
    get_db, get_jobs_by_status, get_jobs_needing_resume,
...
def _check_if_portfolio_sent(messages: list[dict]) -> bool:
    """Check if portfolio URL was already sent in conversation."""
    for msg in messages:
        if msg["sender"] == "me" and PORTFOLIO_URL and PORTFOLIO_URL in msg["text"]:
            return True
    return False
...
def _generate_auto_reply(messages: list[dict], job: dict, config: dict) -> str | None:
    ...
    prompt = f"""你是一位求职者，正在某直聘上和HR沟通。请根据对话上下文生成一条自然、礼貌的回复。
...
def check_replies(config: dict) -> list[dict]:
    ...
    # Get jobs we've sent greetings to (or already replied/resume_sent/follow_up_sent - keep monitoring)
    ...
    all_tracked_jobs = sent_jobs + replied_jobs + resume_sent_jobs + follow_up_jobs
...
def _handle_conversation(job: dict, config: dict) -> str:
    ...
    Returns action taken: 'skipped_user_replied', 'rejected', 'auto_replied', 'resume_sent', 'failed'
    """
    ...
    if _detect_resume_request(messages):
        console.print("[cyan]    HR要求简历，开始处理...[/cyan]")
        ...
        if resume_path:
            console.print(f"[green]    ✓ 简历已生成: {resume_path}[/green]")

        # Step 2: Send platform resume via "发简历" button
        resume_sent = _deliver_resume_to_chat(target_id)
        if resume_sent:
            console.print("[green]    ✓ 平台简历已发送[/green]")

        # Step 3: Send portfolio link if not sent before
        if not _check_if_portfolio_sent(messages):
            time.sleep(2)
            link_msg = f"这是我的在线作品集，方便您查看：{PORTFOLIO_URL}"
            if _send_message_in_chat(target_id, link_msg):
                console.print("[green]    ✓ 作品集链接已发送[/green]")
            else:
                console.print("[yellow]    ! 作品集链接发送失败[/yellow]")

        # Step 4: If HR also asked questions (beyond resume request), reply to them
        hr_new_msgs = _get_hr_messages_after_last_reply(messages)
        non_resume_msgs = [m for m in hr_new_msgs if not any(kw in m["text"] for kw in ["简历", "附件"])]
        if non_resume_msgs:
            console.print("[cyan]    HR还有其他问题，生成回复...[/cyan]")
            time.sleep(3)
            reply = _generate_auto_reply(messages, job, config)
            if reply:
                if _send_message_in_chat(target_id, reply):
                    console.print(f"[green]    ✓ 追问回复已发送[/green]")
                else:
                    console.print("[yellow]    ! 追问回复发送失败[/yellow]")

        # Update status
        db = get_db()
        update_job_status(db, job["id"], "resume_sent")
        add_history(db, job["id"], "resume_sent", f"简历+链接已发送")
        db.close()
...
def _check_follow_ups(config: dict, throttle) -> int:
    ...
    console.print(f"\n[cyan]发现 {len(stale_jobs)} 个超过{interval_hours}小时未回复的对话，尝试跟进...[/cyan]")
...
def _generate_follow_up(job: dict, config: dict) -> str | None:
    ...
    prompt = f"""你是一位求职者，之前在某直聘上给HR发了招呼消息，但{interval_hours}小时没收到回复。请生成一条简短的跟进消息。
...
def monitor_and_send_resumes(config: dict) -> dict:
    ...
    if not window_checker.is_active():
        ...
        return {"skipped": 0, "replied": 0, "resume_sent": 0, "rejected": 0, "failed": 0}

    # Step 1: Check for HR replies
    console.print("\n[bold cyan]═══ 监听HR回复 ═══[/bold cyan]")
    replied_conversations = check_replies(config)

    if not replied_conversations:
        return {"skipped": 0, "replied": 0, "resume_sent": 0, "rejected": 0, "failed": 0}

    console.print(f"[green]检测到 {len(replied_conversations)} 个需要处理的对话[/green]")

    # Step 2: Handle each conversation
    throttle = RequestThrottle(
        throttle_config.get("interval_min", 60),
        throttle_config.get("interval_max", 180),
    )

    summary = {"skipped": 0, "replied": 0, "resume_sent": 0, "rejected": 0, "failed": 0}
    ...
    console.print(f"\n[bold green]═══ 本轮完成 ═══[/bold green]")
    console.print(f"  跳过(已手动回复): {summary['skipped']}")
    console.print(f"  自动回复: {summary['replied']}")
    console.print(f"  简历发送: {summary['resume_sent']}")
    if summary["rejected"]:
        console.print(f"  [dim]拒绝: {summary['rejected']}[/dim]")
    if summary["failed"]:
        console.print(f"  [yellow]失败: {summary['failed']}[/yellow]")

    # Step 3: Follow up on jobs with no reply for 48+ hours
    follow_up_count = _check_follow_ups(config, throttle)
    if follow_up_count:
