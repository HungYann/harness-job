...
# Default DB: <project_root>/data/bosshunter.db
# Override with env var BOSSHUNTER_DB=/custom/path.db
DB_PATH: Path = Path(os.environ["BOSSHUNTER_DB"]) if "BOSSHUNTER_DB" in os.environ else (PROJECT_ROOT / "data" / "bosshunter.db")
...
def get_funnel_stats(conn: sqlite3.Connection) -> dict[str, int]:
    ...
    ai_scored = conn.execute("SELECT COUNT(*) as cnt FROM jobs WHERE status IN ('scored', 'ready', 'approved', 'rejected', 'sent', 'replied', 'resume_sent', 'follow_up_sent')").fetchone()["cnt"]
    approved = conn.execute("SELECT COUNT(*) as cnt FROM jobs WHERE status IN ('approved', 'sent', 'replied', 'resume_sent', 'follow_up_sent')").fetchone()["cnt"]
    sent = conn.execute("SELECT COUNT(*) as cnt FROM jobs WHERE status IN ('sent', 'replied', 'resume_sent', 'follow_up_sent')").fetchone()["cnt"]
    replied = conn.execute("SELECT COUNT(*) as cnt FROM jobs WHERE status IN ('replied', 'resume_sent')").fetchone()["cnt"]
    ...
    return {"采集总数": total, "初筛通过": prefilter_passed, "AI评分": ai_scored, "人工确认": approved, "发送": sent, "回复": replied, "简历已发": resume_sent, "跟进": follow_up, "拒绝": rejected}
...
def get_jobs_needing_resume(conn: sqlite3.Connection) -> list[dict]:
    """Get jobs where HR replied but resume not yet sent."""
    rows = conn.execute(
        "SELECT * FROM jobs WHERE status = 'replied' AND resume_path IS NULL ORDER BY updated_at DESC"
    ).fetchall()
    return [dict(row) for row in rows]
