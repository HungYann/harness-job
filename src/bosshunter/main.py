...
def cli(ctx: click.Context, config_path: str | None) -> None:
    ...
    if ctx.invoked_subcommand is None:
        if _is_first_run(path):
            ...
            # Has config but no subcommand: show brief status hint
...
def connect(ctx: click.Context) -> None:
    ...
    if boss_tab:
        console.print(f"[green]✓[/green] 发现 某直聘 页面: {boss_tab.get('title', '')}")
    else:
        console.print("[yellow]![/yellow] 未发现 某直聘 页面，请在 Chrome 中打开 www.zhipin.com 并登录")
...
def monitor(ctx: click.Context, once: bool, interval: int | None) -> None:
    ...
    if once:
        ...
        console.print(f"\n[bold]本次: 自动回复{summary.get('replied',0)}条, 简历发送{summary.get('resume_sent',0)}份, 跳过{summary.get('skipped',0)}条[/bold]")
    else:
