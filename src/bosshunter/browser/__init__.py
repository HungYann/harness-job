...
def check_chrome_connection(cdp_url: str | None = None) -> dict | None:
    ...
    If *cdp_url* is given, probe that endpoint directly (used by CDPSession).
    Otherwise try the CDP Proxy first, then fall back to the direct port.
    """
    if cdp_url:
        # Caller wants to verify a specific endpoint
        for path in ("/health", "/json/version"):
            try:
                resp = httpx.get(f"{cdp_url.rstrip('/')}{path}", timeout=3)
